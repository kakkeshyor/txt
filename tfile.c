...
void _printTHeader(THeader *h, FILE *stream) {
  fprintf(stream, "%-2s %-2s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s\n",
          "T", "BO", "B", "NT", "BT", "ND", "BD", "SUMB", "SUMU", "SUMTF");
  fprintf(stream, "%-2u %-2u %-10u %-10u %-10u %-10u %-10u %-10u %-10u %-10u\n",
          h->type, h->bo, h->b, h->nt, h->bt, h->nd, h->bd, h->sumb, h->sumu,
          h->sumtf);
}

void _printTDocHeader(TDocHeader *h, FILE *stream) {
  fprintf(stream, "%-2s %-2s %-10s %-10s %-10s %-10s %-10s %-10s %-16s\n", "T",
          "BO", "ID", "SUMB", "SUMU", "SUMTF", "MAXTF", "SUMSQTF",
          "SUMSQLOGTF");
  fprintf(stream, "%-2u %-2u %-10u %-10u %-10u %-10u %-10u %-10u %+1.8le\n",
          h->type, h->bo, h->id, h->sumb, h->sumu, h->sumtf, h->maxtf,
...
int packTHeader(uchar *buf, THeader *h) {
  return _pack(buf, "cciiiiiiii", h->type, h->bo, h->b, h->nt, h->nd, h->bt,
               h->bd, h->sumb, h->sumu, h->sumtf);
}
...
int packTDocHeader(uchar *buf, TDocHeader *h) {
  return _pack(buf, "cciiiiiif", h->type, h->bo, h->id, h->sumb, h->sumu,
               h->sumtf, h->maxtf, h->sumsqtf, h->sumsqlogtf);
}

int packTDoc_payload(uchar *buf, TDoc *tdoc) {
  return _pack(buf, "II", tdoc->txt, tdoc->h->sumu, tdoc->tf, tdoc->h->sumu);
}

int unpackTHeader(THeader *h, int n, uint8_t *buf) {
  int m;
  if ((m = _unpack(buf, "cciiiiiiii", &h->type, &h->bo, &h->b, &h->nt, &h->nd,
                   &h->bt, &h->bd, &h->sumb, &h->sumu, &h->sumtf)) != n)
    return -1;
  assert(h->type == 0x01);
  return m;
}
...
int unpackTDocHeader(TDocHeader *h, int n, uint8_t *buf) {
  int m;
  if ((m = _unpack(buf, "cciiiiiif", &h->type, &h->bo, &h->id, &h->sumb,
                   &h->sumu, &h->sumtf, &h->maxtf, &h->sumsqtf,
                   &h->sumsqlogtf)) != n)
    return -1;
  assert(h->type == 0x02);
  return m;
...
int unpackTDoc_payload(TDoc *tdoc, int n, uint8_t *buf) {
  int m;
  if ((m = _unpack(buf, "II", tdoc->txt, tdoc->h->sumu, tdoc->tf,
                   tdoc->h->sumu)) != n)
    return -1;
  return m;
}
...
TFile *readTFile(FILE *fp, TFile *tfile) {
  ...
  for (int i = 0; i < tfile->h->nd; i++) {
    ...
    if ((n = unpackTDocHeader(tdoc->h, BTDOCHEADER, buf)) != BTDOCHEADER)
      eprintf(
          "unpackTDocHeader() failed on %d byte TDocHeader for doc %d of %d\n",
          BTDOCHEADER, i, tfile->h->nd);
