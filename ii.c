  ...
  */
  ...
  for (int j = 0; j < tdq->h->sumu; j++) {
    ...
    for (Node *npp = term->plist; npp != NULL; npp = npp->next) {
       ...
       * query */
      tf  = (*m.tf)(post->tf, d->sumb, d->sumu, d->sumtf, d->maxtf, d->sumsqtf,
                   d->sumsqlogtf, tfc->h->sumb, tfc->h->sumu, tfc->h->sumtf,
                   tfc->h->nd);
      df  = (*m.df)(term->df, tfc->h->nd);
      qtf = (*m.qtf)(tdq->tf[j], d->sumb, d->sumu, d->sumtf, d->maxtf,
                     d->sumsqtf, d->sumsqlogtf, tfc->h->sumb, tfc->h->sumu,
                     tfc->h->sumtf, tfc->h->nd);
