...
int cmppost(void *d1, void *d2) { return ((Post *)d1)->id - ((Post *)d2)->id; }
...
int cmpterm(void *d1, void *d2) { return ((Term *)d1)->id - ((Term *)d2)->id; }
...
int cmphit(void *d1, void *d2) { return ((Hit *)d1)->id - ((Hit *)d2)->id; }
...
double SMART_n_n_tf(uint32_t tf, uint32_t sumb, uint32_t sumu, uint32_t sumtf,
                    ...
                    uint32_t sumb_, uint32_t sumu_, uint32_t sumtf_,
                    uint32_t nd) {
  return (double)tf;
}
...
double SMART_d_b_tf(uint32_t tf, uint32_t sumb, uint32_t sumu, uint32_t sumtf,
  ...
  */
  ...
  double tf_;
  tf_ = (1.0 + log(1.0 + log((double)tf))) /
        (0.8 * ((double)sumb_ / (double)nd) + 0.2 * (double)sumb);
  return tf_;
}
...
double SMART__n__df(uint32_t df, uint32_t nd) { return 1.0; }
...
double OKAPI_BM25_tf(uint32_t tf, uint32_t sumb, uint32_t sumu, uint32_t sumtf,
  ...
  */
  ...
  double tf_;
  tf_ = 3.0 * (double)tf /
        (0.4 + 1.6 * (double)sumb * (double)nd / (double)sumb_ + (double)tf);
  return tf_;
}
